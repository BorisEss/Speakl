...
class ThreeColumnsTableView: UIView {
    ...
    private var subtitleMiddle: String = ""
    ...
    private func loadData(title: String,
        ...
        self.data = data
        self.title = title
        self.subtitleLeft = subtitles[optional: 0] ?? ""
        self.subtitleMiddle = subtitles[optional: 1] ?? ""
        self.subtitleRight = subtitles[optional: 2] ?? ""
        ...
        checkMinimalIndex(&bottomRightIndex, defaultIndex: IndexPath(row: data.count + 1, column: 2))
...
extension ThreeColumnsTableView: SpreadsheetViewDataSource {
...
extension ThreeColumnsTableView: SpreadsheetViewDelegate {
    func spreadsheetView(_ spreadsheetView: SpreadsheetView, cellForItemAt indexPath: IndexPath) -> Cell? {
        ...
        if let cell = mainCell as? TextCell {
            ...
            case IndexPath(row: 0, column: 0): cell.setUp(color: .speaklYellowLabel,
                                                          ...
                                                          radius: 10,
                                                          text: title)
            case IndexPath(row: 1, column: 0): cell.setUp(color: .speaklGreen,
                                                          text: subtitleLeft)
            case IndexPath(row: 1, column: 1): cell.setUp(color: .speaklGreen,
                                                          text: subtitleMiddle)
            case IndexPath(row: 1, column: 2): cell.setUp(color: .speaklGreen,
                                                          text: subtitleRight)
            ...
            case bottomRightIndex: cell.setUp(color: .speaklBlueSolid,
                                              ...
                                              radius: 10,
                                              text: data[indexPath.row - 2][optional: 2])
            default:
    ...
    func mergedCells(in spreadsheetView: SpreadsheetView) -> [CellRange] {
        var range = [CellRange(from: (0, 0), to: (0, 2))]
        range += mergedItems
        return range
    }
