//
//  TwoColumnsGrouppedTableView.swift
//  stori
//
...
class TwoColumnsGrouppedTableView: UIView {
    ...
    init(width: CGFloat,
        ...
        self.loadData(title: data.title ?? "", data: data.data ?? [])
    }
    ...
    private func loadData(title: String, data: [[String]]) {
        self.data = data
        self.title = title
        ...
        checkMinimalIndex(&bottomLeftIndex, defaultIndex: IndexPath(row: data.count, column: 0))
        checkMinimalIndex(&bottomRightIndex, defaultIndex: IndexPath(row: data.count, column: 1))
...
extension TwoColumnsGrouppedTableView: SpreadsheetViewDataSource {
...
extension TwoColumnsGrouppedTableView: SpreadsheetViewDelegate {
    func spreadsheetView(_ spreadsheetView: SpreadsheetView, cellForItemAt indexPath: IndexPath) -> Cell? {
        ...
        if let cell = mainCell as? TextCell {
            ...
            case bottomLeftIndex:
                cell.setUp(color: indexPath.column == 0 ? .speaklGreen : .speaklBlueSolid,
                           roundedCorners: [.bottomLeft],
                           radius: 10,
                           text: data[indexPath.row - 1][optional: 0])
            case bottomRightIndex:
                cell.setUp(color: indexPath.column == 0 ? .speaklGreen : .speaklBlueSolid,
                           roundedCorners: [.bottomRight],
                           radius: 10,
                           text: data[indexPath.row - 1][optional: 1])
            default:
                cell.setUp(color: indexPath.column == 0 ? .speaklGreen : .speaklBlueSolid,
                           text: data[indexPath.row - 1][optional: indexPath.column])
            }
            return cell
        }
        return mainCell
    ...
    func spreadsheetView(_ spreadsheetView: SpreadsheetView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.column == 1,
            let item = data[indexPath.row - 1][optional: indexPath.column] {
            didSelectItem?(item)
        }
    }
